...
__usage__ = """
					...
					optional:
					--min_contig_len <INT, minimal_length_to_be_considered_for_plot>[1000000]
...
def get_coverage( input_vcf, high_name, low_name ):
	...
	return sorted( coverage_high )[ int( len( coverage_high ) / 2 ) ], sorted( coverage_low )[ int( len( coverage_low ) / 2 ) ]


def get_delta_allel_frequencies( input_vcf, output_file, unique_variant_vcf, high_name, low_name, avg_cov_high, avg_cov_low ):
						...
						if parts[6] == "PASS" and not "," in parts[4]:	#avoid triallelic variants:
							...
							if h_cov >= min_high_cov and h_cov <= max_high_cov and l_cov >= min_low_cov and l_cov <= max_low_cov:
								...
								if af_high_status + af_low_status < 2:
									...
									out.write( "\t".join( parts[:7] +  list( map( str, [ ".", ".", h_cov, h_allele1, h_allele2, l_cov, l_allele1, l_allele2, af_delta ] ) ) ) + '\n' )
							elif h_cov >= min_high_cov and h_cov <= max_high_cov and l_cov == 0:
								unique_out.write( "\t".join( parts[:7] + list( map( str, [ ".", ".", h_cov, h_allele1, h_allele2, l_cov, l_allele1, l_allele2, 1 ] ) ) ) + '\n' )
...
def plot_genome_wide_delta_allele_frequencies( af_frequency_vcf, unique_variant_vcf, chr_lengths, window_size, step_size ):
	...
	for idx, each in enumerate( chr_names ):
		...
		ax.scatter( data_to_plot_x[ idx ], data_to_plot_y[ idx ], c=list( map( abs, data_to_plot_y[ idx ] ) ), s=1, cmap="cool" )
		...
		plt.subplots_adjust( left=0.03, right=0.98, top=0.9, bottom=0.2 )
...
def plot_genome_wide_single_pos_dAF( af_frequency_vcf, unique_variant_vcf, chr_lengths ):
	...
	for idx, each in enumerate( chr_names ):
		...
		# --- adding variant information --- #
		ax.scatter( raw_data_x[ idx ], raw_data_y[ idx ], c=list( map( abs, raw_data_y[ idx ] ) ), s=1, cmap="cool" )	#hot, binary
		
		# --- adding unique variant information --- #
		ax.scatter( additional_raw_data_x[ idx ], additional_raw_data_y[ idx ], c=list( map( abs, additional_raw_data_y[ idx ] ) ), s=1, cmap="cool", marker="+" )
...
def load_seq_lengths( multiple_fasta_file, seq_len_cutoff, variant_counts ):
...
def main( arguments ):
	...
	if '--min_contig_len' in arguments:
		seq_len_cutoff = int( arguments[ arguments.index( '--min_contig_len' ) + 1 ] )
	else:
		seq_len_cutoff = 1000000
	...
	get_delta_allel_frequencies( input_vcf, af_frequency_vcf, unique_variant_vcf, high_name, low_name, cov_high, cov_low )
	
	construct_delta_AF_frequency_hist( af_frequency_vcf )
	
	variant_counts = count_variants( input_vcf )
	chr_lengths = load_seq_lengths( fasta_ref_file, seq_len_cutoff, variant_counts )
	print( chr_lengths )
